	...
	mInputReader(32)
{
	...
	mPendingEvents[Gyroscope    ].gyro.status = SENSOR_STATUS_ACCURACY_HIGH;
	...
	mPendingEvents[LinearAcc    ].version = sizeof(sensors_event_t);
	mPendingEvents[LinearAcc    ].sensor = ID_LA;
	mPendingEvents[LinearAcc    ].type = SENSOR_TYPE_LINEAR_ACCELERATION;
...
int AkmSensor::setEnable(int32_t handle, int enabled)
{
	...
	if ((id == Accelerometer)||(id == MagneticField)) {
		...
		int tmp = mEnabled[Gyroscope]
			+ mEnabled[Orientation]
			+ mEnabled[Gravity]
			+ mEnabled[LinearAcc]
			+ mEnabled[RotationVector];
		if (enabled) {
			if (tmp <= 0) {
				buffer[0] = '1';
			}
...
void AkmSensor::processEvent(int code, int value)
{
	switch (code) {
		...
		case EVENT_TYPE_GYRO_STATUS:
			mPendingMask |= 1<<Gyroscope;
			mPendingEvents[Gyroscope].gyro.status = value;
			break;
		...
		case EVENT_TYPE_GRAVITY_X:
			mPendingMask |= 1<<Gravity;
			mPendingEvents[Gravity].data[0] = value * CONVERT_GR;
			break;
		case EVENT_TYPE_GRAVITY_Y:
			mPendingMask |= 1<<Gravity;
			mPendingEvents[Gravity].data[1] = value * CONVERT_GR;
			break;
		case EVENT_TYPE_GRAVITY_Z:
			mPendingMask |= 1<<Gravity;
			mPendingEvents[Gravity].data[2] = value * CONVERT_GR;
			break;

		case EVENT_TYPE_LACCEL_X:
			mPendingMask |= 1<<LinearAcc;
			mPendingEvents[LinearAcc].data[0] = value * CONVERT_LA;
			break;
		case EVENT_TYPE_LACCEL_Y:
			mPendingMask |= 1<<LinearAcc;
			mPendingEvents[LinearAcc].data[1] = value * CONVERT_LA;
			break;
		case EVENT_TYPE_LACCEL_Z:
			mPendingMask |= 1<<LinearAcc;
			mPendingEvents[LinearAcc].data[2] = value * CONVERT_LA;
			break;
