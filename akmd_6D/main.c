...
#define ERROR_GET_SIZE_DOEEX    (-10)
#define ERROR_MALLOC_DOEEX    (-11)
...
void Disp_MeasurementResultHook(AKSCPRMS* prms, const uint16 flag)
{
	...
	rbuf[9] = CONVERT_FLOAT_Q16(prms->m_pgout.u.x);	/* Gy_x */
	rbuf[10] = CONVERT_FLOAT_Q16(prms->m_pgout.u.y);/* Gy_y */
	rbuf[11] = CONVERT_FLOAT_Q16(prms->m_pgout.u.z);/* Gy_z */
	...
	rbuf[16] = CONVERT_FLOAT_Q16(prms->m_pgGravity.u.x);
	rbuf[17] = CONVERT_FLOAT_Q16(prms->m_pgGravity.u.y);
	rbuf[18] = CONVERT_FLOAT_Q16(prms->m_pgGravity.u.z);
	/* LinAcc (from Float to Q16 format) */
	rbuf[19] = CONVERT_FLOAT_Q16(prms->m_pgLinAcc.u.x);
	rbuf[20] = CONVERT_FLOAT_Q16(prms->m_pgLinAcc.u.y);
	rbuf[21] = CONVERT_FLOAT_Q16(prms->m_pgLinAcc.u.z);
	/* RotVec (from Float to Q16 format) */
	rbuf[22] = CONVERT_FLOAT_Q16(prms->m_pgquat.u.x);
	rbuf[23] = CONVERT_FLOAT_Q16(prms->m_pgquat.u.y);
	rbuf[24] = CONVERT_FLOAT_Q16(prms->m_pgquat.u.z);
	rbuf[25] = prms->m_pgquat.u.w;
	AKD_SetYPR(rbuf);
	/*
		ALOGE("oceanacc acc(dec)=%6d,%6d,%6d\n",
						prms->m_avec.u.x, prms->m_avec.u.y, prms->m_avec.u.z);

		ALOGE("oceanmag mag(dec)=%6d,%6d,%6d\n",
						prms->m_hvec.u.x, prms->m_hvec.u.y, prms->m_hvec.u.z);
						
		ALOGE("oceanori Yaw, Pitch, Roll=%6.1f,%6.1f,%6.1f\n",
...
int main(int argc, char **argv)
{
    ...
    doeex_size = AKSC_GetSizeDOEEXVar();
    if (doeex_size <= 0) {
        retValue = ERROR_GET_SIZE_DOEEX;
        goto THE_END_OF_MAIN_FUNCTION;
    }
    prms.m_doeex_var = (AKSC_DOEEXVAR *)malloc(doeex_size * sizeof(int32)); /* -> prms.m_doeex_var should be defined as the member of AKSCPRMS in AKCompass.h. */
    if (prms.m_doeex_var == NULL) {
        retValue = ERROR_MALLOC_DOEEX;
        goto THE_END_OF_MAIN_FUNCTION;
    }
...
#if 1
				msleep(100);				
				memset(prms.m_doeex_var, 0, doeex_size * sizeof(int32));
				//InitAKSCPRMS(&prms);//test
				LoadPDC(&prms);
				if (LoadParameters(&prms) == 0) {
					SetDefaultPRMS(&prms);
				}
	...
	free(prms.m_doeex_var);
	prms.m_doep_var = NULL;
	prms.m_doeex_var = NULL;
