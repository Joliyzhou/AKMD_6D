...
int16 LoadAccSTDEV(const char* path, double *x, double *y, double *z)
{
	int16	ret;
	FILE	*fp = NULL;
	char	keyName[KEYNAME_SIZE];	
	...
	ret = 1;
#if 1
	snprintf(keyName, sizeof(keyName), "ACC_X_STDEV");
	ret = ret && LoadDouble(fp, keyName, &acctmp);
	*x = acctmp;
	acctmp = 0.0f;
	snprintf(keyName, sizeof(keyName), "ACC_Y_STDEV");
	ret = ret && LoadDouble(fp, keyName, &acctmp);
	*y = acctmp;
	acctmp = 0.0f;
	snprintf(keyName, sizeof(keyName), "ACC_Z_STDEV");
	ret = ret && LoadDouble(fp, keyName, &acctmp);
	*z = acctmp;
	acctmp = 0.0f;
	//ALOGE("AKM ocean load acc STDEV : %20lf,%20lf,%20lf\n",*x,*y,*z);
#endif
	if (fclose(fp) != 0) {
		AKMERROR_STR("fclose");
		ret = 0;
